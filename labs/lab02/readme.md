...
Ответ: порт находится на коммутаторе с более низким приоритетом моста.
...
Протокол STP выбрал вышеуказанные порты в качестве портов корневого моста исходя из PortID соседа.

Вопрос: какое значение протокол STP использует первым после выбора корневого моста, чтобы определить выбор порта?

Ответ: путь до корневого моста (Path Cost), чем ниже стоимость, тем предпочтительнее будет порт.
...
Ответ: самое низкое значение bridge ID
